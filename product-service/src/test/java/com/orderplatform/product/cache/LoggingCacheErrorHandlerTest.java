...
class LoggingCacheErrorHandlerTest {
    ...
    void handleCacheGetError_doesNotRethrow() {
        ...
        assertThatCode(() -> handler.handleCacheGetError(new RuntimeException("Redis down"), cache, 1L))
    ...
    void handleCachePutError_doesNotRethrow() {
        ...
        assertThatCode(() -> handler.handleCachePutError(new RuntimeException("Redis down"), cache, 1L, "value"))
                .doesNotThrowAnyException();
    }
    ...
    void handleCacheEvictError_doesNotRethrow() {
        ...
        assertThatCode(() -> handler.handleCacheEvictError(new RuntimeException("Redis down"), cache, 1L))
                .doesNotThrowAnyException();
    }
    ...
    void handleCacheClearError_doesNotRethrow() {
        Cache cache = mock(Cache.class);
        when(cache.getName()).thenReturn("productList");
